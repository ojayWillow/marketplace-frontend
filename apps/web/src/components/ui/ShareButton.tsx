...
const ShareButton = ({
   ...
   * Uses window.open() for all platforms — this reliably triggers
   * deep links (WhatsApp, Telegram) on mobile and opens popups on
   * desktop. Using window.location.href for deep links caused the
   * browser to stall on many mobile devices and PWAs.
   */
  const openShareUrl = (shareUrl: string, popupOptions?: string) => {
    window.open(shareUrl, '_blank', popupOptions || 'noopener,noreferrer');
    closeSheet();
  };
  ...
  const shareToFacebook = () => {
    ...
    openShareUrl(shareUrl, 'noopener,noreferrer,width=600,height=400');
  };
