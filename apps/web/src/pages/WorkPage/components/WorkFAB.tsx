...
const WorkFAB = () => {
  ...
  const content = (
    <>
      ...
      {/* FAB container — sits just above the bottom nav */}
      <div
      ...
      >
        {isOpen && (
          <>
            ...
            <div className="flex items-center gap-2 animate-fade-in-up" style={{ animationDelay: '0ms' }}>
              <span className="px-2.5 py-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-xs font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap">
                {t('work.fab.postJob', 'Publicēt darbu')}
              </span>
              <Link
                to="/tasks/create"
                className="w-10 h-10 flex items-center justify-center bg-blue-500 text-white rounded-full shadow-lg hover:bg-blue-600 transition-colors"
                onClick={() => setIsOpen(false)}
              >
            ...
            <div className="flex items-center gap-2 animate-fade-in-up" style={{ animationDelay: '50ms' }}>
              ...
              <Link
                to="/offerings/create"
                className="w-10 h-10 flex items-center justify-center bg-amber-500 text-white rounded-full shadow-lg hover:bg-amber-600 transition-colors"
                onClick={() => setIsOpen(false)}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          className={`w-11 h-11 flex items-center justify-center rounded-full shadow-lg transition-all duration-200 ${
            isOpen
              ? 'bg-gray-500 dark:bg-gray-600 rotate-45'
