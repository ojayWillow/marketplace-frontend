    ...
    <div>
      ...
      <div className="relative mb-2 z-[1000]">
        ...
        <input
          ...
          className="w-full pl-8 pr-28 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base sm:text-sm placeholder:text-gray-400 dark:placeholder:text-gray-500"
          autoComplete="off"
        />
        ...
        {navigator.geolocation && (
          <button
            ...
            disabled={geoLoading}
            className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-md transition-colors disabled:opacity-50"
          >
            {geoLoading ? (
              <>
                <span className="animate-spin">⏳</span>
                <span className="hidden sm:inline">{t('createTask.detecting', 'Detecting...')}</span>
              </>
            ) : (
              <>
                <span>📍</span>
                <span className="hidden sm:inline">{t('createTask.useMyLocation', 'My Location')}</span>
                <span className="sm:hidden">{t('createTask.useMyLocationShort', 'My Loc')}</span>
              </>
            )}
          </button>
        )}
