  ...
  return (
    <>
      ...
      <div className={`bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 ${compact ? 'p-3' : 'p-4 md:p-6'}`}>
        ...
        {/* View Toggle */}
        {!viewOnly && (
          <div className={`flex gap-1 mb-3 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg ${compact ? 'w-full' : 'w-full md:w-fit'}`}>
            <button
              ...
              className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-sm font-medium transition-all relative ${
                taskViewMode === 'my-tasks' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-600 dark:text-gray-400'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 md:flex-none px-3 py-1.5 rounded-md text-sm font-medium transition-all relative ${
                taskViewMode === 'my-jobs' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-600 dark:text-gray-400'
              }`}
            >
                ...
                return (
                  <div 
                    ...
                    className={`${compact ? 'p-3' : 'p-3 md:p-4'} border rounded-lg transition-colors ${
                      hasApplications ? 'border-green-300 dark:border-green-700 bg-green-50 dark:bg-green-900/10' : 
                      hasMatches ? 'border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-900/10' :
                  ...
                  >
                    ...
                    <div>
                      <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                        ...
                        <Link to={`/tasks/${task.id}`} className="font-medium text-sm text-gray-900 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400 line-clamp-1">
                          {task.title}
                        </Link>
                        {!viewOnly && (
                      ...
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <Link to={`/tasks/${task.id}`} className="px-2.5 py-1 text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/30 font-medium">
                          {t('profile.jobsTab.view')}
                        </Link>
                        {!viewOnly && task.status === 'open' && !hasApplications && (
                          <>
                            <Link to={`/tasks/${task.id}/edit`} className="px-2.5 py-1 text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 font-medium">
                              {t('profile.jobsTab.edit')}
                            </Link>
                            {onCancelTask && (
                              <button onClick={() => onCancelTask(task.id)} className="px-2.5 py-1 text-xs text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-md hover:bg-red-100 dark:hover:bg-red-900/30 font-medium ml-auto">
                ...
                return (
                  <div 
                    ...
                    className={`${compact ? 'p-3' : 'p-3 md:p-4'} border rounded-lg ${
                      isPending ? 'border-yellow-200 dark:border-yellow-800/40 bg-yellow-50/30 dark:bg-yellow-900/10' :
                      needsReview ? 'border-yellow-200 dark:border-yellow-800/40 bg-white dark:bg-gray-900' : 
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <Link
                          to={`/tasks/${task.id}`}
                          className="px-2.5 py-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/30 font-medium"
                        >
                          {t('profile.jobsTab.view')}
                        </Link>
                      </div>
