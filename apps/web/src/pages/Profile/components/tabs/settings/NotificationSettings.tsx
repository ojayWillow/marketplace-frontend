...
export const NotificationSettings = () => {
    ...
    if (next) {
      ...
      try {
        ...
        if (geoErr?.code === 1) {
          ...
          setJobAlertError(t('common.notifications.locationUnavailable', 'Your browser does not support geolocation.'));
        } else {
          setJobAlertError(t('common.notifications.locationError', 'Could not determine your location. Please try again.'));
        }
      }
    } else {
      setIsJobAlertExpanded(false);
  ...
  const getSummary = () => {
    if (!pushIsOn) return t('settings.notifications.statusOff', 'Disabled');
    const parts: string[] = [t('settings.notifications.statusOn', 'Enabled')];
    if (jobAlertPrefs.enabled) {
      ...
      <div className={`transition-all duration-300 overflow-hidden ${expanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pb-4 space-y-3 border-t border-gray-100 dark:border-gray-700 pt-3">
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800/40">
              ...
              <div>
                ...
                <ol className="text-xs text-blue-700 dark:text-blue-400 mt-1 space-y-0.5 list-decimal list-inside">
                  <li>{t('settings.notifications.iosStep1', 'Tap Share in Safari')}</li>
                  <li>{t('settings.notifications.iosStep2', 'Add to Home Screen')}</li>
                  <li>{t('settings.notifications.iosStep3', 'Open from home screen')}</li>
                </ol>
              </div>
            </div>
          )}
          ...
          {isIOSPWA() && !isSupported && (
            <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800/40">
              ...
              <p className="text-xs text-yellow-800 dark:text-yellow-300">
                {t('settings.notifications.iosVersionRequired', 'Requires iOS 16.4+')}
              </p>
            </div>
          )}
          ...
          {!isSupported && !showIOSHelp && !isIOSPWA() && (
            <div className="flex items-center gap-2 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800/40">
              ...
              <p className="text-xs text-yellow-800 dark:text-yellow-300">
                {t('settings.notifications.notSupported', 'Not supported by your browser.')}
              </p>
            </div>
          )}
            ...
            <div className="flex items-center gap-2 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800/40">
              ...
              <div>
                ...
                <p className="text-[10px] text-red-600 dark:text-red-400 mt-0.5">
                  {t('settings.notifications.blockedHelp', 'Allow notifications in browser settings.')}
                </p>
              </div>
            </div>
          )}
          ...
          {canShowToggle && (
            <div className="flex items-center justify-between">
              ...
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow-sm ${pushIsOn ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
            </div>
          )}

          {pushError && (
            <p className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-2 rounded-lg">{pushError}</p>
          )}
          ...
          {pushIsOn && (
            <div className="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-700">
              ...
              <div className="rounded-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
                {jobAlertLoading ? (
                  <div className="flex items-center gap-2 text-gray-400 dark:text-gray-500 px-3 py-2">
                  ...
                  <div className="flex items-center gap-2 px-3 py-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20">
                    <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    {t('common.notifications.jobAlertLoadError', 'Could not load job alert preferences.')}
                  </div>
                ) : (
                  <>
                    {/* Job alert header */}
                    <div
                      ...
                      className={`flex items-center justify-between px-3 py-2.5 ${jobAlertPrefs.enabled ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50' : ''}`}
                    >
                      <div className="flex items-center gap-2.5 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-gray-500 dark:text-gray-400 truncate">
                            {jobAlertPrefs.enabled
                              ? (isJobAlertExpanded
                                  ? t('common.notifications.jobAlertsDesc', 'Get notified about new tasks near you')
                                  : `${jobAlertPrefs.radius_km} km \u00b7 ${jobAlertPrefs.categories.length === 0 ? t('common.notifications.allCategories', 'All categories') : `${jobAlertPrefs.categories.length} ${t('common.notifications.categoriesCount', 'categories')}`}`
                                )
                              : t('common.notifications.jobAlertsDesc', 'Get notified about new tasks near you')}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {jobAlertPrefs.enabled && (
                          <svg className={`w-3.5 h-3.5 text-gray-400 dark:text-gray-500 transition-transform duration-200 ${isJobAlertExpanded ? 'rotate-180' : ''}`}
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                          </svg>
                        )}
                    ...
                    {jobAlertError && (
                      <div className="mx-3 mb-2 flex items-start gap-2 p-2 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800/40">
                        ...
                        <p className="text-xs text-red-700 dark:text-red-400">{jobAlertError}</p>
                      </div>
                    )}

                    {jobAlertSaved && (
                      <div className="mx-3 mb-2 flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400">
                        <svg className="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                        <span>{t('common.notifications.jobAlertsSaved', 'Saved!')}</span>
                      </div>
                    )}

                    {/* Expandable content */}
                    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${jobAlertPrefs.enabled && isJobAlertExpanded ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
                      {/* Radius */}
                      ...
                      {/* Categories */}
                      <div className="px-3 pt-2 mt-2 border-t border-gray-100 dark:border-gray-700 pb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {TASK_CATEGORIES.map((cat) => {
                            ...
                            return (
                              ...
                              >
                                <span className="text-xs" role="img" aria-label={cat}>
                                  {CATEGORY_ICONS[cat] || '\ud83d\udccb'}
                                </span>
                        ...
                        <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1.5">
                          {t('common.notifications.maxCategories', 'Select categories to filter. Leave empty for all.')}
                        </p>
                      </div>
                    </div>
                  </>
              ...
              <div className="pt-2 border-t border-gray-100 dark:border-gray-700">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-lg transition-colors disabled:opacity-50"
                >
                  {testLoading ? (
