    ...
    return (
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{t('profile.reviewsTab.title')}</h3>
        </div>
        <div className="text-center py-4">
          <span className="text-2xl">⭐</span>
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">{t('profile.reviewsTab.noReviews')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">
          ...
          <span className="text-gray-400 dark:text-gray-500 font-normal ml-1">({reviews.length})</span>
        </h3>
      </div>
            ...
            {editingReview === review.id ? (
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg text-sm"
                />
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setEditingReview(null)} className="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md text-xs">{t('profile.reviewsTab.cancel')}</button>
                </div>
              </div>
            ) : (
              <div className="flex items-start gap-2.5">
                <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
                  {review.reviewer_avatar ? (
                    ...
                    <span className="text-gray-500 dark:text-gray-400 text-xs font-medium">{review.reviewer_name?.charAt(0).toUpperCase()}</span>
                  )}
                </div>
