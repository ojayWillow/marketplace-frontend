  ...
  <div className="mx-4 mb-3 md:mx-6 md:mb-5 p-3 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
    <div className="flex items-center justify-between gap-2">
      <div className="flex-1 min-w-0">
        <h3 className="font-bold text-xs text-amber-800 dark:text-amber-300 flex items-center gap-1 mb-0.5">
          🚀 Boost Visibility
        </h3>
        {offering.is_boost_active && boostTimeRemaining ? (
          <p className="text-xs text-green-700 dark:text-green-400 font-medium">✅ Active • {boostTimeRemaining} left</p>
        ) : (
          <p className="text-xs text-amber-700 dark:text-amber-400">Show on map for 24h</p>
        )}
      </div>
      {offering.is_boost_active && boostTimeRemaining ? (
        <span className="px-2.5 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-lg font-bold text-xs">🔥 Boosted</span>
      ) : (
        <button
          onClick={onBoost}
          disabled={isBoosting}
