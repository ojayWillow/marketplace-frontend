...
const MessageInput = ({
  ...
  useEffect(() => {
    ...
    const maxHeight = isMobile ? 100 : 120; // ~4 lines
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    // Enter sends (without Shift); Shift+Enter inserts newline
        ...
        >
          ...
          {onImageSend && (
            <>
              <button
                ...
                className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full text-blue-500 hover:bg-blue-50 active:bg-blue-100 disabled:opacity-40 transition-colors mb-px"
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              autoFocus={!isMobile}
              className={`w-full resize-none rounded-3xl border border-gray-200 bg-gray-50 px-4 py-2.5 focus:outline-none focus:border-blue-400 focus:bg-white focus:ring-1 focus:ring-blue-100 transition-colors placeholder:text-gray-400 ${
          ...
          {/* Send button — circular blue with arrow icon */}
          <button
            type="submit"
            disabled={!canSend}
            className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all mb-px ${
              canSend
